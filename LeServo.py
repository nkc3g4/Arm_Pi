...
class PWM_Servo(object): #class定义一个类，object在python3中默认加
    ...
    def __init__(self, pi, pin, freq = 50, min_width = 500, max_width=2500, deviation=0, control_speed=False):
        ...
        if control_speed is True: #当运行速度改变时
            thread.start_new(PWM_Servo.updatePosition,(self,)) #(self, )只有一个数值时必须加一个逗号
            #调用thread模块中的start_new_thread()函数来产生新线程，第一个参数为执行函数，第二个参数为前面函数的参数
    ...
    def updatePosition(self):
        while True:
            if self.posChanged is True: #角度有变化
                ...
                self.incTimes = self.Time / self.stepTime #分步次数
                if self.positionSet > self.Position: #目标角度和上一次的目标角度进行比较
                    self.positionInc = self.positionSet - self.Position #差值
                    self.positionInc = -self.positionInc #取反
                else:
                    self.positionInc =  self.Position - self.positionSet #差值
                self.positionInc = self.positionInc / float(self.incTimes) #每步转动的角度
