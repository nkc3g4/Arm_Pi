...
import signal
import LeArm
import kinematics_paper as kin
...
color_dist = {#'red': {'Lower': np.array([0, 160, 160]), 'Upper': np.array([6, 255, 255])},
              ...
              'white': {'Lower': np.array([20, 0, 170]), 'Upper': np.array([125, 20, 255])},
              }
# 色块颜色，位置列表
position_color_list = []
# 识别到色块标志
    ...
    if correction_flag is False:
		...
		print("HSV at center:",hsv[img_center_y,img_center_x])
	            ...
	            #cv2.imshow('mask', mask)
                    ...
                    if len(cnts) > 0:
                        # 找出最大的区域
			tmp_list = sorted(cnts, key=cv2.contourArea)
			...
			c = tmp_list[-1];
                        ...
                        h, w = rect[1]
                        c_angle = rect[2]
			print('abs h-w',abs(h-w))
			if len(cnts)>1 and not(h * w >= 1250 and h >= 30 and w >= 30 and h < 90 and w < 90 and c_y<200 and abs(h-w)<15):
			    c = tmp_list[-2];
                            # 返回的值 中心坐标（x, y）,（w，h）,角度
                            rect = cv2.minAreaRect(c)
                            # 获取最小外接矩形的4个顶点
                            ...
                            c_angle = rect[2]
                            print('abs h-w',abs(h-w))
                        if h * w >= 1250 and h >= 30 and w >= 30 and h < 90 and w < 90 and c_y<200 and abs(h-w)<15:   # 色块面积限制
			    print('h,w',h,w)
                            # 绘制中心点
